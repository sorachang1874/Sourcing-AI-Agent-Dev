...
repeat profile-query variants. The authoritative result-v3 schema can persist handle, stable platform user ID,
...
Before emitting the final object, audit every retained handle against the authored-Post/authored-Reply pair and run a
separate unresolved-pretraining gap pass. This audit is an evidence-deepening obligation, not a business candidate or
call cap. It should upgrade candidates when public technical evidence exists while preserving honest ambiguous or
unsupported states when both scoped searches are zero-yield.
