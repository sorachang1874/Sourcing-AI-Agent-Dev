...
class AdaptiveGrokWaveRunnerTests(unittest.TestCase):
    ...
    def test_current_bundle_requires_hash_bound_process_result_journal(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                _bytes_sha(journal_raw),
            )
            journal = json.loads(journal_raw)
            self.assertEqual(
                journal["stdout_sha256"],
                _bytes_sha((run_root / "raw.stdout").read_bytes()),
            )
            self.assertEqual(journal["stdout_bytes"], (run_root / "raw.stdout").stat().st_size)
            self.assertEqual(
                journal["stderr_sha256"],
                _bytes_sha((run_root / "stderr.txt").read_bytes()),
            )
    ...
    def _assert_promoted_post_consumption_target_is_pre_mutation(
        self,
        artifact_key: str,
    ) -> None:
        self.assertIn(artifact_key, runner._POST_CONSUMPTION_ARTIFACT_KEYS)
        with tempfile.TemporaryDirectory() as directory:
            ...
            auth_sha = state["auth_sha"]
            intent = json.loads(
                (run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["operator_intent"]).read_text()
            )
            stdout_spool = run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["stdout_spool"]
            stderr_spool = run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["stderr_spool"]
            session_updates = runner._session_updates_path(
                run_root / intent["runtime_layout"]["ephemeral_home_name"],
                run_root / "workspace",
                intent["command_binding"]["session_id"],
            )
            promoted_raw = {
                "raw_stdout": stdout_spool.read_bytes(),
                "stderr": stderr_spool.read_bytes(),
                "session_updates": session_updates.read_bytes(),
                "sanitized": stdout_spool.read_bytes(),
            }[artifact_key]
            state["consumption_path"].unlink()
            state["active_claim"].unlink()
            for evidence_key in (
                ...
                if evidence_path.exists() or evidence_path.is_symlink():
                    evidence_path.unlink()
            promoted_path = run_root / runner._RUN_ARTIFACT_NAME_REGISTRY[artifact_key]
            _atomic_publish(promoted_path, promoted_raw)
            run_snapshot = _tree_snapshot(run_root)
            approvals_snapshot = _tree_snapshot(approvals)
            process_group_is_alive = mock.Mock(return_value=False)
            process_group_identity_matches = mock.Mock(return_value=True)
            ...
            self.assertEqual(_tree_snapshot(run_root), run_snapshot)
            self.assertEqual(_tree_snapshot(approvals), approvals_snapshot)
            self.assertEqual(promoted_path.read_bytes(), promoted_raw)
            self.assertFalse(state["active_claim"].exists())
            self.assertFalse(runner._auth_digest_is_tainted(approvals, auth_sha))

    def test_recovery_promoted_raw_stdout_is_pre_mutation(self) -> None:
        self._assert_promoted_post_consumption_target_is_pre_mutation("raw_stdout")

    def test_recovery_promoted_stderr_is_pre_mutation(self) -> None:
        self._assert_promoted_post_consumption_target_is_pre_mutation("stderr")

    def test_recovery_promoted_session_updates_is_pre_mutation(self) -> None:
        self._assert_promoted_post_consumption_target_is_pre_mutation("session_updates")

    def test_recovery_promoted_sanitized_output_is_pre_mutation(self) -> None:
        self._assert_promoted_post_consumption_target_is_pre_mutation("sanitized")

    def test_recovery_promoted_raw_stdout_defeats_legacy_origin_rewrite(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            state = _incomplete_live_run_after_executor_return(
                Path(directory),
                spawn=True,
            ...
            auth_sha = state["auth_sha"]
            stdout_spool = run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["stdout_spool"]
            promoted_raw = stdout_spool.read_bytes()
            state["consumption_path"].unlink()
            claim = json.loads(state["active_claim"].read_text())
            ...
            for evidence_key in (
                ...
                if evidence_path.exists() or evidence_path.is_symlink():
                    evidence_path.unlink()
            promoted_path = run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["raw_stdout"]
            _atomic_publish(promoted_path, promoted_raw)
            run_snapshot = _tree_snapshot(run_root)
            approvals_snapshot = _tree_snapshot(approvals)
            process_group_is_alive = mock.Mock(return_value=False)
    ...
    def test_recovery_promoted_terminal_receipt_retains_terminal_early_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            run_root, approvals = _completed_live_run(Path(directory))
            request = json.loads(
                (run_root / runner._RUN_ARTIFACT_NAME_REGISTRY["operator_request"]).read_text()
            )
            grant_id_hash = _bytes_sha(request["approval"]["grant_id"].encode())
            _, consumption_path = runner._grant_paths(approvals, grant_id_hash)
    ...
    def test_recovery_genuine_pre_consumption_crash_retains_legacy_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse(active_claim.exists())
            for artifact_key in runner._PRE_CONSUMPTION_PENDING_ARTIFACT_KEYS:
                self.assertTrue(
                    (run_root / runner._RUN_ARTIFACT_NAME_REGISTRY[artifact_key]).is_file(),
                    artifact_key,
                )
            for name in (
                "process-result.json",
    ...
    def test_journal_publish_failure_preserves_claim_and_recovery_seals_from_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def fail_process_result(path: Path, value: bytes, *, pre_publish: Any = None) -> None:
                if path.name == "process-result.json":
                    raise OSError("synthetic_process_result_publish_failure")
                original_publish(path, value, pre_publish=pre_publish)

            with mock.patch.object(runner, "_atomic_publish", side_effect=fail_process_result):
                with self.assertRaisesRegex(OSError, "synthetic_process_result_publish_failure"):
                    _run_adaptive_wave(
                        request=request,
                        execution_mode="live",
                        runtime_root=root / "runtime",
            ...
            self.assertTrue((run_root / "process-ledger.json").is_file())
            self.assertTrue((run_root / ".stdout-spool").is_file())
            self.assertFalse((run_root / "process-result.json").exists())
            ...
            self.assertEqual(journal["phase"], "recovery_sealed")
            self.assertEqual(
                recovered["artifacts"]["process_result_journal_sha256"],
                _bytes_sha(journal_path.read_bytes()),
            )
            self.assertFalse((run_root / "ephemeral-home").exists())
            self.assertFalse(active_claim.exists())
            self.assertEqual(validate_operator_bundle(run_root, approval_root=approvals), [])

            journal["timed_out"] = True
            _write_private(journal_path, (canonical_json(journal) + "\n").encode())
            self.assertIn(
    ...
    def test_unconfirmed_result_without_process_identity_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(AdaptiveWaveValidationError, "process_result_unrecoverable"):
                _run_adaptive_wave(
                    request=request,
                    execution_mode="live",
                    runtime_root=root / "runtime",
    ...
    def test_session_sealing_exception_preserves_home_claim_and_transcript(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            os.chmod(root, 0o700)
            binary, auth, binary_sha = _live_material(root)
            ...
            run_root = next((root / "runtime").iterdir())
            active_claim = approvals / f"auth-active-use-{auth_sha}.json"
            source_updates = runner._session_updates_path(
                run_root / "ephemeral-home",
                run_root / "workspace",
                json.loads((run_root / "operator-intent.json").read_text())["command_binding"]["session_id"],
            )
            self.assertTrue(source_updates.is_file())
            self.assertTrue((run_root / "ephemeral-home/auth.json").is_file())
            self.assertTrue(active_claim.is_file())
            self.assertTrue((run_root / "process-result.json").is_file())
            self.assertFalse((run_root / "session-updates.jsonl").exists())
            ...
            self.assertEqual(recovered["status"], "crash_recovered")
            self.assertEqual(recovered["session_proof"]["status"], "verified")
            self.assertTrue((run_root / "session-updates.jsonl").is_file())
            self.assertFalse((run_root / "ephemeral-home").exists())
            self.assertFalse(active_claim.exists())
            self.assertEqual(validate_operator_bundle(run_root, approval_root=approvals), [])

    def test_session_publication_oserror_preserves_recoverable_source_transcript(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            os.chmod(root, 0o700)
            binary, auth, binary_sha = _live_material(root)
            ...
            def fail_retained_updates(path: Path, value: bytes, *, pre_publish: Any = None) -> None:
                if path.name == "session-updates.jsonl" and path.parent.name.startswith("grok_wave_live_"):
                    raise OSError("synthetic_session_publication_failure")
                original_publish(path, value, pre_publish=pre_publish)

            with mock.patch.object(runner, "_atomic_publish", side_effect=fail_retained_updates):
                with self.assertRaisesRegex(OSError, "synthetic_session_publication_failure"):
                    _run_adaptive_wave(
                        request=request,
                        execution_mode="live",
                        runtime_root=root / "runtime",
...
"""
    ...
    def test_post_executor_publication_failure_preserves_auth_until_recovery(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            os.chmod(root, 0o700)
            binary, auth, binary_sha = _live_material(root)
