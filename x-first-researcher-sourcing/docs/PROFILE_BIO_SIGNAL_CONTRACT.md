...
| Ecosystem registry | `profile_bio_signal_policy.v1.json`, policy v1.4 | Canonical hash-pinned aliases, positive ownership templates, positive continuation classes, and a closed parenthetical-note registry; there is no negative-phrase denylist |
...
relevance, these outputs can expand the human verification queue for China-related professional experience. V1.4
...
- Affiliation relations: `current`, `previous`. V1.4 has no positive grammar or output for `unspecified`.
...
- Ecosystem support requires the complete NFKC-normalized proposal claim window to match one registered positive
  ...
  `（长文首发）` are the only continuation classes. Arbitrary parenthetical prose is not a positive continuation.
...
- Current affiliation accepts only the versioned positive grammar IDs for `Head of <role> @handle`,
  `Founder/Researcher/Engineer at @handle`, `任职于 @handle`, and `就职于 @handle`. Previous affiliation accepts only
  the versioned `Prev/Previously/Formerly @handle...`, `Former <role> @handle`, `曾任 @handle...`, `前任职于 @handle`,
  and `此前任职于 @handle` forms. The complete window must match exactly one grammar for the declared relation.
...
value. Policy v1.4 records only positive ownership continuations, positive current/previous affiliation grammar IDs,
the exact parenthetical-note registry, exact parsed-role binding, rejected Unicode categories, and validation traversal
budgets. It contains no negative-phrase or temporal denylist. Any alias, ownership template, continuation class,
positive affiliation grammar, role rule, limit, forbidden field, or output-state change under the
