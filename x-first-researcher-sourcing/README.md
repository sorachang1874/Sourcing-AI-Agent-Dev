...
| Bio and mention enrichment | Fixture-only schema v1 with policy v1.1 separates observed Chinese content, anchored closed-grammar China-ecosystem leads, organization proposals and graph edges; it rejects every native/live mode until a separately reviewed profile-capability v2 trust root exists |
...
The executable Bio v1 accepts only `.invalid` fixture profiles, null tool receipts, and canonical policy v1.1. Its
ownership and affiliation clauses reject registered negation/recruiting contexts, and malformed nested JSON returns
validation errors instead of escaping the validator. It cannot be relabelled as proof that Grok returned a Bio or
