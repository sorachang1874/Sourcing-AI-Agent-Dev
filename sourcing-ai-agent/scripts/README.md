...
| 脚本 | 用途 |
|---|---|
| `ws7_shadow_divergence_report.py` | 议案①（batch divider）+ 议案③（promote judge）影子路径的**接合证据**与**规则梯 vs scripted 分歧语料**。三个子命令：`engagement`（跑真实生产接缝——`queue_background_profile_prefetch` / `build_company_candidate_artifacts`——在自建并自动销毁的 `sourcing_test_ws7_*` schema 里产出真实 shadow record；**seam 真、数据合成**，只算 PATH 证据）、`divider`（只读回放 live registry 的 wave-scoped 成员集 + 磁盘快照 roster，跑梯子与 scripted 分割，出 V1–V10 电池结果、按精确成员集去重的计数、与梯子同 R5 口径的弃置量对比）、`promote`（只读回放 `organization_asset_registry` 的 realistic 36 / extended 806 对 + 5 个事故场景，extended 每对整行落盘）。**fail-closed**：三重门 env 任一存在即拒跑；只读连接强制 `default_transaction_read_only = on`（由 PG 而非纪律保证）；只用 scripted 确定性客户端，绝不走 `build_model_client`。**输入侧诚实性（divider）：live registry 中 `refill_queue_state='ready'` 的行数为 0，harness 对每个成员伪造 `ready` 队列态**——真实的是 url population/shard token/尝试史，队列态不是；报告以 `input_reconstruction` 块（含实时 census）披露，「ready set」一律读作「重构集」。**其余诚实性红线：scripted 客户端只能证明『路径通了』和『真正触发过的那些验收器的行为』（divider 侧只有 V1），证明不了任何 AI 判断质量；`ai_more_permissive` 是 guard∧电池地板放行数的上界论证，不是关于真模型的比较断言；真模型语料是 operator-gated 的独立步骤（S6）。** |
