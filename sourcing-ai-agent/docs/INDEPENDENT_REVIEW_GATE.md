...
Use the canonical runner's Codex app-server JSON-RPC transport in read-only mode. The standing policy is the newest available model at its highest supported reasoning effort. The operator maintains the reviewer-only model, effort, and service tier in `configs/reviewer-codex/reviewer.toml`. `make independent-review-gate` first builds `runtime/reviewer_codex_home_v2/`: its private `config.toml` is derived from the current source Codex config with that checked-in top-level policy trio forced, while login/session state is symlinked back to the source home. The Make target then supplies this directory as `CODEX_HOME`, so ChatGPT Desktop may continue changing `~/.codex/config.toml` without changing reviewer routing. The bootstrap reads but never writes the source home, uses private directory/file permissions, and fails without replacing any same-name real target entry. This is config isolation, not full Codex-state isolation.
