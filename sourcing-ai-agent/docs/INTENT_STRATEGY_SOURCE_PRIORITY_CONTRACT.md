...
- **CONTRACT REVISION (WS1 Step 3, 2026-07-22; operator recall directive 2026-07-19)**: organization profile defaults NEVER steer `strategy_type` — the org-profile steering branches are retired from `_determine_strategy_decision`. A directional query is `scoped_search_roster` because of the QUERY shape (size-agnostic fallback rule), never because of company size; non-directional queries take the full-roster path for every company. `org_scale_band` and the profile survive only as advisory shard-parameter metadata (paging budgets, probe settings, snapshot reuse limits). Coverage-driven promotion (`scoped_directional` + proven full-company coverage) is unchanged, and the Google sub-org scope preference (2026-07-20 directive) survives as a SCOPE rule: a sub-org-scoped query (e.g. Google DeepMind / Gemini) takes the multi-company-page full-roster path for recall regardless of directionality — post-retirement this holds with or without a registry profile row (previously the profile branch shadowed it whenever a registry row existed).
- **CONTRACT REVISION (WS1 Step 4c, 2026-07-23)**: the duplicated `_infer_strategy_type` inference ladder is RETIRED from `acquisition_strategy.py`. Explicit metadata wins — the explicit override, `use_company_employees_lane` preference, investor population, and former-only population are each decided once by the explicit branches of `_determine_strategy_decision`; the ladder re-derived them from raw fields and could fork on normalization drift. Only the live fallback tail remains, spelled out with per-rule reason codes alongside the stable `fallback_rule_strategy` family code: `fallback_rule_google_sub_org_scope_roster` (the Google/Alphabet sub-org SCOPE rule above), `fallback_rule_multi_scope_hint_scoped` (a non-directional request still carrying 2+ scope hints stays scoped — only reachable when a semantic brief overrides directionality), and `fallback_rule_company_default_full_roster` (everything else). Strategy outcomes are unchanged; the reason codes are additive audit detail.
