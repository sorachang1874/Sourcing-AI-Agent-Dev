...
- [~] **B4 — 策略 Step 4 ＋ 切片 3 回涨归位＋棘轮 ＋ 拆分第二波**（mypy B/C 已随 81→0 完成）
  - 切片 3 **防再涨棘轮 DONE 2026-07-22**：`tests/test_monolith_regrowth.py` 入 lane——七个单体文件行数 shrink-only（orchestrator 82,300 起）,captured-improvement 强制（低于预算 >600 行必须同批缩预算）。回涨带归位（track-d action schema ~2,018 行/C1b claims ~1,576 行→owner 模块）待做——棘轮先立,归位时逐批缩预算。
