...
- **遗留删除条件**（pgLegacy 硬计数 @69b7423，删除前重测）：`adaptive_us_technical_partition` 是**当前写默认**（`company_shard_planning.py:385`）——先改写默认+双接受各比较位点（acquisition.py:4849/5125/5322/5383/5428/7568；asset_reuse_planning.py:2263/2832），read-side shim 到终态行老化；no-policy 回退（`acquisition.py:2123-2129`）删除条件 = 终态化 8 条 stalled running 行；acquire_former_search_seed 路径删除条件 = 裁决 2 个 stale pending plan_review_sessions。**两删除条件均已满足 2026-07-23**（8 行终态化 cancelled/superseded + governance 标注,#2/#7 经 store.review_plan_session 规范口 cancelled;硬计数重测全零：nonterminal_jobs=0/running_workflows=0/pending_reviews=0/policyless_roster_jobs=0,pin `5a8f0b3`）。**⚠️ 删除缓行发现（2026-07-23）**：`test_request_scoped_roster_shards.py` RosterLaneExecutionTest 多数用例以 policy-less 任务行使该分支——per-function 探针分片的批准合同正经由 no-policy→request-plan-shards 采纳路径被 pin,分支兼任 legacy 回退与现行合同载体。删除切片必须同批：①裁定执行期 policy-less 目标语义（fail-closed blocked vs request-plan 单写者继续）——注意纯删会让任务滑入更宽的 unsharded root 路径,fail-closed 才是隐藏回退移除的正确形态；②迁移套件到显式 policy 元数据;③过评审门（隐藏回退移除触发器）。
