...
- [~] **B2 进行中**：开局件 DONE `5c1d085`（tick oracle 基线 10/10 复验；test_pipeline 冻结棘轮入 lane 461 测试/42,416 行只减；Phase 4 两文档锚点勘误横幅+关键锚重测 run_worker_recovery_once=40442-42605；Block (a) 前移决定入档）。**余**：级联簇迁移本体（profile_refill 链/workflow_resume 波/projection 串行/followup 尾环 → recovery_phases.py，oracle byte-identical 硬门，逐段重测锚点）＋ test_pipeline 打捞移植（planning/retrieval/normalize-snapshot 簇 → PG fixture 新文件，冻结棘轮同批缩减）。**③ playbook 回流首批 DONE**：doc 25（shrink-only ratchets + forensic test repair + cost-profile probes）已推 `121d117`→GitHub main；预检两条策略告警待 operator（shell 代理 env 已设置；Clash GLOBAL=DIRECT）。**打捞第一波 DONE** `9ae85d4`：4 个 plan_workflow 意图推断测试移植到 `test_orchestrator_planning.py`（PG fixture 重座 4/4 绿），god-file 461→457/42,309 行棘轮同批缩减，溯源映射入 REGRESSION_INDEX——打捞循环全链路已验证；第 5 候选（intent-axes normalization，引用已消失的 keyword_priority_only 键）留二波法证
