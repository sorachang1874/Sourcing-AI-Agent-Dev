...
- **Step 5（架构议案，先设计后动）**——**双裁决 RATIFIED 2026-07-22（operator 亲答）**：①归一形状=**全并入 company_shard_planning**（cohort 成为 shard planner 的输入模式,CohortProviderCompiler 退役;既有 cohort 身份/授权/选择契约——cohort_selection 的显式 cohort 权威、execution identity、refinement 栅栏——必须在并入中完整存活并由并入后套件逐条 pin）；②investor_firm_roster **留在抽象外**,作为独立 strategy 显式契约化（统一 artifact schema 中以 source_kind=funding_graph 标记为抽象外例外,不留隐式特例）。**下一步=Step 5 设计文档**（并入蓝图:cohort 输入模式的 planner 接口、契约保存矩阵、迁移阶梯、T-002 墓碑件的归宿、退役 CohortProviderCompiler 的调用面清单）→ 评审门 → 分片实施。
