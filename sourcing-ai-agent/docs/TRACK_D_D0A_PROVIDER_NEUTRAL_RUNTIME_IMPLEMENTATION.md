...
| Session projections | `ToolCallingSessionBase` plus `ParsedToolTurn` | One subclass-owned canonical parse outcome; base returns its terminal result and yields its complete advisory tuple | Separate buffered/stream parsing, result reconstruction from events, terminal omission/replacement, or subclass projection override | A future live subclass may strengthen admission context but must preserve this one-outcome boundary | abstract/probe method identity, terminal coherence, scripted parity, and pre-consumption route-fence tests |
