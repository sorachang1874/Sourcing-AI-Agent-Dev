...
S1e0 (`d05a073`) characterizes the unratified generic approval/dispatch/budget surfaces. S1e1 decision-locks the
...
zero, and generic approve/dispatch do not expand. S1e2a now implements the first specialized submit UoW behind native
adapter/repository delegates: current occurrence/mode/full-root preflight runs before adapter access; one finite
