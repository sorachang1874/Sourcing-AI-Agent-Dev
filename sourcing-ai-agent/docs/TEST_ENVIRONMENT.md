...
> Revision note (2026-07-23, R-037 recovery-coverage parity): `isolated_hosted_test_runtime(...)` now starts the SAME in-process shared recovery service `serve` uses in its dev-compat mode (`start_shared_recovery_service` + `assert_recovery_coverage_or_fail_closed`, stopped and joined on teardown). Rationale: the API process is signal-only (Step 5e) and `serve` fail-closes without a recovery driver (Step 5a); the harness previously ran uncovered, so scripted acquisition workers stayed queued forever and jobs never reached the terminal result-view + run-scope-projection assembly (hosted smoke results then 410'd behind the serving-finalized fail-closed gate). The runtime dataclass exposes the coverage verdict as `recovery_coverage`. Coverage: `tests/test_scripted_runtime_teardown.py::...::test_isolated_hosted_test_runtime_provides_recovery_coverage`.
