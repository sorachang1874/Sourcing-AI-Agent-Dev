...
| unlinked Plan history identity | API authentication state 是唯一 owner；`submit_plan_workflow` 只把 server provenance + exact `requester_id`/`tenant_id` 证明写入 history metadata | authenticated submit 覆盖 client identity/provenance；first-create 省略 `history_id` 并要求响应给出非空 server ID；revision 发送既有 server ID；read/list/resubmit 必须同时匹配 `authenticated_request_state_v1`、requester 与 `user-<id>` tenant | Plan submit、frontend local snapshot rekey/route/poll、frontend-history read/list/revision submit | authenticated mode 下 explicit absent ID、missing/partial/mismatch proof、missing response ID 或 revision ID drift 一律 fail closed；不得信任 body、姓名或 legacy metadata 推断 owner；旧 non-Plan unlinked row 可保持 read compatibility，但不能因此获得 Plan replacement authority；open mode 保持兼容 | C1b process-local bridge；C1c durable consumer identity 上线且 legacy unresolved inventory 清零后删除 metadata proof |
...
asserts that no direct hydration-run caller exists, and inspects thread, `submit`, `asyncio.to_thread`,
`run_in_executor`, and pool `starmap` targets; it fails if another queue, thread/executor hydration target, API compile
caller, or CLI callsite appears. Authenticated initial submit keeps its client UUID local-only, omits it from the API
body, atomically rekeys the local cache/route/session to the returned server ID, and only revisions send that existing
ID. Queue registration and owner retirement share one lock and owner token, so a late
...
- Agent/Operation 调试面板必须消费后端的 typed operation/runtime contract，而不是从 command 名称猜测能力
  ...
  - `POST /api/operations/actions` 的 fresh success 只返回 `queued|approval_required` + HTTP 202；preflight-observed exact replay 或已返回非 fresh lifecycle 的 committed replay 返回 `idempotent_replay=true` + HTTP 200，并保留 persisted current status。submit adapter 的闭集为 `approval_required|queued|planned|running|completed|failed|cancelled|rejected`，其中 `rejected` 仅供 brownfield action row；未知状态必须 fail closed，前端不得把 replay 强制改写成 `queued`。`OperationActionSubmitResponse` 是 submit 专用判别联合：fresh 分支必须携带 literal `idempotent_replay=false`，replay 分支必须携带 literal `true`；replay-only 状态缺失、伪造或携带 false 标记一律拒绝。generic `OperationActionDetailResponse` 仅供 GET/decision 共用，标记可选；前端不得从 HTTP code、action id 或嵌套 run 状态反推 replay
