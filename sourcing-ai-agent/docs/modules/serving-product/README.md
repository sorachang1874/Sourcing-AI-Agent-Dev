...
| Filter v2 fixed-forward lineage decision | [TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md](decisions/TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md) | S1f0c FF-DI canonization author candidate; append-only literal lock, 22 full-schema contract digests with acyclic dependency DAG, design-evidence closure map, structured eight-row join, six PG relations under three owners with typed descriptors, migration `0015` reservation, product terminal core/envelope digests and V3 result mapping; machine contract [filter_projection_lineage_fixed_forward_decision_v1.json](contracts/filter_projection_lineage_fixed_forward_decision_v1.json); no runtime/served delta, scope-matched review pending |
