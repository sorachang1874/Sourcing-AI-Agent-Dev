...
| Filter v2 fixed-forward lineage decision | [TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md](decisions/TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md) | S1f0c FF-DI canonization author candidate; append-only literal lock, 22 full-schema contract digests with acyclic dependency DAG, two retained root/owner-ref closed-schema pins with exact-version-and-digest references, design-evidence closure map, 92 machine-executed eight-row join predicates with per-predicate hostile mutations, six PG relations under three owners with typed descriptors and one-to-one invariant enforcement (exact digest constants/bounds, composite scoped FKs, repository-enforced classification), migration `0015` reservation, product terminal core/envelope digests, V3 exact union discrimination with closed item schemas and tool digest binding; machine contract [filter_projection_lineage_fixed_forward_decision_v1.json](contracts/filter_projection_lineage_fixed_forward_decision_v1.json); no runtime/served delta, scope-matched review pending |
