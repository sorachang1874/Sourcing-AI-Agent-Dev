...
| Filter v2 fixed-forward lineage decision | [TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md](decisions/TRACK_D_D1N_S1F0C_LINEAGE_FIXED_FORWARD_DECISION.md) | S1f0c FF-DI canonization author candidate; append-only literal lock, 22 full-schema contract digests with acyclic dependency DAG, two retained root/owner-ref closed-schema pins with exact-version-and-digest references, design-evidence closure map, 100 machine-executed eight-row join predicates (digest-pinned external preview validator, full requester-binding scope comparisons with bidirectionally total comparison_links audit maps including the AcquisitionRun bundle carrier/workspace predicates, per-predicate hostile mutations), six PG relations under three owners with typed descriptors and one-to-one invariant enforcement (exact digest constants/bounds, composite scoped FKs, repository-enforced classification, opaque `membership_revision` text token, lane provider-evidence UoW binding, terminal append-only transition_guard split), migration `0015` reservation, product terminal core/envelope digests, V3 exact union discrimination with closed item schemas, closed `cohort_selection` object, and complete ActionResultSpec/AgentToolSpec fingerprint decision-lock (64-KiB/8192-item/depth-10 limits, empty artifact schemes, owner-pin digest recomputation) plus exactly materialized canonical_fingerprints (real JSON nulls, empty provider-mode array, all canonical keys; request pin binds the retained projection_filter_request_v2 identity and V2 binder with executable parity, no invented V3 request contract; oracle constructs real runtime specs, compares recursively and type-strictly, and pins result_schema_digest/tool_spec_digest); machine contract [filter_projection_lineage_fixed_forward_decision_v1.json](contracts/filter_projection_lineage_fixed_forward_decision_v1.json); no runtime/served delta, scope-matched review pending |
