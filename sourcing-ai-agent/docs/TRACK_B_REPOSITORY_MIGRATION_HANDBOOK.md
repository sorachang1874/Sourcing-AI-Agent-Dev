...
- **②.3b 新增经验**:(a) `projection_manifest_shards` 看似普通行映射,但读路径把负数/坏整数 clamp 为 0,且 `_row_value`
  对任意 `row[key]` 异常回默认值;generic `Kind.INT` 与共享窄异常 row helper 都不等价,故保留 bespoke mapper + repo-local helper;
