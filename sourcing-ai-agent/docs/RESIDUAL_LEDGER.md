...
| R-011 | lint 门 mypy 段 | **87 条 pre-existing mypy 债务**（candidate_artifacts 1 / orchestrator 64 / public_web_runtime_core 1 / workflow_smoke 21；2026-07-06 基线） | 继承债务，非本期修复范围 | 分模块清偿 | **棘轮：只降不增** —— 每批验收 = 与基线计数逐字相同或更小；下降时同批重钉基线 | ②.1 基线；②.2 / ②.3a / ②.3b 复验均为 87（1/64/1/21） | 2026-07-10 | accepted |
...
| R-014 | Track B ②.3b independent review | 首个独立 Codex reviewer 在完整 artifact 写入时遭 model-capacity failure；其 chat 曾回报证据充分/GO，但 chat-only 结果无效；两个 fallback session 亦未在本 session 产出 artifact | reviewer backend 容量/收口中断，不是 repo/network/provider 问题；代码/验证已提交且 scope 固定 | 容量可用时由独立非作者 Codex 对 `b3818f0..aaf13fb` 重跑/收口，产完整 metadata + findings + GO/NO-GO artifact | 异步重试；在有效 GO 前只阻断 ②.3b 的 live/W6/manual/里程碑签收，其他开发继续 | author evidence：A/B 3+9、pinned hash `74cc0b13...d0f00`、direct 21+2+7、lane 190/0、mypy 87/4；无有效 review artifact | 2026-07-10 | pending async Codex review |
