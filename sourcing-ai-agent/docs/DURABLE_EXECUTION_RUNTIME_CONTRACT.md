...
- Entity-delta identity is command+entity scoped, not attempt scoped. Producers derive `idempotency_key` (and therefore `delta_id`) from the command id plus entity identity (`entity_type`/`entity_key`/`delta_kind`), so a retried command that re-finalizes and re-records the same logical outcome hits the write-once path and receives the persisted row unchanged (first-writer-wins idempotent replay). `attempt_id` on a delta is provenance about which attempt first recorded the outcome; it is deliberately NOT part of the immutable identity, because a later attempt of the same command legitimately rewrites it (2026-07-20 incident: `linkedin.profile_refill.submit_batch` attempt=2 crashed the recovery daemon with `immutable identity collision ... : attempt_id`). The immutable identity fence still fails closed on genuine conflicts: same `delta_id`/`idempotency_key` with a different `workspace_id`, `workflow_run_id`, `operation_run_id`, `command_id`, `activity_run_id`, `acquisition_run_id`, `entity_type`, `entity_key`, or `delta_kind` raises and must never be weakened into an upsert.
...
Remote-wait worker recovery is terminal-event first, orphan-poll second. A submitted remote-wait worker (`waiting_remote_harvest` / `waiting_remote_search` with a recorded `run_id`/`dataset_id`) is owned by the terminal-event path — the provider webhook or the in-process long-poll watcher records `remote_provider_terminal_event` on its checkpoint, and only then does the worker recovery daemon claim and resume it for dataset collection. The daemon must NOT eagerly re-poll fresh remote-wait workers (provider cost and single-poller ownership). However, the event owner is not durable: the watcher thread dies with its serve process and a webhook may be absent, so a worker whose `updated_at` is older than `WORKER_RECOVERY_REMOTE_WAIT_ORPHAN_SECONDS` (default 900) is treated as orphaned and admitted through the normal claim/resume path, bounded by `WORKER_RECOVERY_REMOTE_WAIT_ORPHAN_LIMIT` (default 4) per tick and reported via `remote_wait_orphan_admitted_count` / `remote_wait_orphan_admitted_worker_ids` in the daemon summary. Each admission performs exactly one provider status poll: a terminal run is downloaded and completed in place (no resubmission, queue summaries flip to `completed`, blocked `acquiring` jobs become resume-ready), while a still-running run is re-queued, which refreshes `updated_at` and self-rate-limits the next poll. `remote_wait_orphan_seconds <= 0` disables the safety net. This closes the 2026-07-20 gap where two full-roster jobs stayed `blocked acquiring` forever because their terminal segmented `harvest_company_employees` shard runs were never collected after a serve restart.
