...
- Model adjudication is a bounded provider activity with fail-visible fallback. OpenAI-compatible relay health must verify the configured model through `/chat/completions`; `/models` alone is inventory evidence, not readiness proof. Health probes are not allowed to become recurring LLM workload: normal healthcheck responses must be cached, and any failed chat completion must open a provider/base-url/model circuit breaker so repeated health probes, recovery ticks, or AI adjudication retries fail fast locally during cooldown. When a model call fails, returns empty text, returns non-JSON, or is blocked by the circuit breaker, the owner may use deterministic fail-closed fallback to avoid unsafe promotion, but the result and phase metrics must carry `provider`, `model`, `model_version`, `fallback_used=true`, `fallback_reason`, and `model_error` or response preview. Readers, live validation, and Agent surfaces must treat that as incomplete AI adjudication evidence, not as successful model review.
