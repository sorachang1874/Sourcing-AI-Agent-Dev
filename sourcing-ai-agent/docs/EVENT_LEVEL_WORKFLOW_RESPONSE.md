...
- Remote provider event handling must wake the already-owned job-scoped recovery service after enqueueing/confirming that service. The wakeup request is a durable local-service signal that interrupts the daemon's idle poll sleep; it is not permission for the webhook/callback to scan the registry, claim URLs, submit providers, run local apply, or publish board patches inline.
- Wakeup payload merging is part of the recovery contract. Concurrent terminal provider events for the same service must union `remote_provider_event_worker_ids` into `explicit_worker_ids`, and the effective `total_limit` may rise only to the bounded remote-event burst limit. A later wakeup may narrow scalar options, but it must not overwrite earlier targeted worker ids; worker ids beyond the burst limit stay in the service's retained wakeup payload and drain on immediate follow-up ticks rather than waiting for a later poll interval.
...
- Remote-provider wakeups are small-step by contract. Multiple terminal worker wakeups may be merged into one service wakeup file, but the job-scoped daemon must not increase `total_limit` to the merged worker count. It passes at most `total_limit` explicit workers into one recovery callback and retains the remaining wakeup worker ids for later ticks. This keeps callback/daemon/recovery latency bounded without dropping terminal provider events.
...
- `src/sourcing_agent/orchestrator.py`
  - remote provider event -> job-scoped recovery -> completed reconcile 的锁范围、marker 写入、same-kind defer。
...
- The full-snapshot materialization daemon owns `snapshot_full_materialization`. Remote-provider-event/job-scoped recovery disables this phase by default because profile batch completion should make cards visible before compaction. Shared/background service ticks may run it separately when the item is ready. Open `snapshot_full_materialization` items are diagnostic/background backlog for a terminal or board-visible job; they must not count as daemon-owned open work for the job-scoped recovery sidecar.
- Job-scoped recovery owns same-job workflow resume after daemon-owned work is clear. Its default is bounded explicit resume for the target job, not a stale-only takeover. A completed Stage 1 candidate document with `acquisition_stage.task_type='enrich_linkedin_profiles'` or `enrichment_scope='linkedin_stage_1'` is reusable only when its profile-prefetch registry proof is terminal (`all_requested_terminal=true` or equivalent requested-vs-terminal counts). Queued/open Stage 1 candidate documents are partial artifacts and must not prevent recovery from continuing LinkedIn Stage 1.
...
- Apify webhook 入口现在支持 `POST /api/providers/apify/webhook`，HTTP 层只做 token 校验、事件规范化、terminal event 写入、lease wakeup，并同步启动或确认唯一的 job-scoped recovery owner；完整 recovery/materialization 不在 webhook response 内执行。`?sync=1` 只保留为低并发调试兼容入口，不能作为 browser/scripted 压力测试主路径，否则会把完整 recovery/materialization 绑进 webhook response 并放大 remote-event lag。
- Recovery owner state machine is single-owner:
  - `webhook` / local watcher / progress polling / shared recovery may only write wakeup evidence, terminal provider events, and recovery service bootstrap requests.
...
- 一批 provider run 被本地观察到后，当前 tick 可以继续唤醒同 job 的 remote-wait worker，而不是等下一轮外层 daemon tick。
...
- recovery summary 明确记录 follow-up scope、worker ids、job ids、claimed/executed counts，便于审计。
...
- webhook 后台线程不应执行 full materialize/reconcile；它只唤醒 job-scoped recovery。下游 materialize 仍由 writer budget 和 background reconcile 追平。
...
- Remote-provider-event recovery is a bounded-burst control loop. A provider terminal event wakes job-scoped recovery with `WORKFLOW_REMOTE_EVENT_RECOVERY_TOTAL_LIMIT=4` by default, so a burst of already-terminal remote workers can be ingested without spreading a single provider completion wave over many poll ticks. This limit is still a hard per-tick bound: callbacks only mark/wake, heavy registry refill, local apply, and board-visible work remain daemon-owned phases, and worker ids beyond the burst limit are retained for immediate follow-up ticks.
...
Public Web 这条 lane 不是 provider webhook；它现在复用相同 registry 让 DataForSEO queued task 的 `waiting_remote_search` worker 被 shared recovery 发现后，立即触发同 job-scoped follow-up。W7 后 normal path 使用 `crm_public_web_search`；`target_candidate_public_web_search` 仅是 migration/legacy lane。
...
- orchestrator unit test: shared recovery tick 后，会对 LinkedIn Stage 1 remote-wait worker 做 job-scoped follow-up。
