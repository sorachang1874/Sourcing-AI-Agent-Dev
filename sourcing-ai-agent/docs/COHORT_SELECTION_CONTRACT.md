...
> Status: CS1 contract, CS2 deterministic provider compiler, CS3 isolated non-live runtime adapter, CS4 user interaction, the CS5 scripted service E2E, and the criteria-write provenance fixed-forward are implemented. The CS3/CS5 formal review of `60e7e67` was `NO-GO 0/6/4/0`; pinned advisories of `04dd41c` and `d7cb306` returned `0/2/2/0` and `0/1/1/0`. The Cohort path-filtered portion of combined commit `fd50a84` is the current author fixed-forward awaiting fresh pinned review. `simulate`/`replay`/`scripted` workflows delegate to the compiled Harvest lane boundary and persist one canonical search-seed/result audit contract; the default-off frontend picker round-trips explicit selections through submit/revision/recovery/review. Paid live execution and the retrieval-only `run_job` helper remain fail-closed.
...
The workflow acquisition runtime now delegates explicit cohorts to that boundary only when the server-owned runtime mode is `simulate`, `replay`, or `scripted`. The explicit-Cohort fence runs before legacy full-roster, scoped, former, investor, or caller-mutated strategy dispatch. New plans emit one full manifest task and no second former task. A hydrated legacy former task exact-recompiles and compares the plan-owned manifest before any snapshot load, lane backfill, merge, or projection writer. It then reads the committed durable full-manifest result without merging/persisting and may reuse it only when compiler inputs, lane digests, result digest, and the complete former-lane set match; it never recompiles or executes a narrower provider plan.
...
- The live Harvest submit function is a hard-failing sentinel through all joined background work. The public `get_job_results_api()` projection is linked through `run_projection_link`, has complete profile readiness, reports clean contamination, and records scripted mode for every provider invocation. The E2E reads the durable workflow current state and exact `CompletionProofRecorded` event for `serving_finalized=proved`; durable job events also prove both queued and terminal outreach reconcile.
- Second fixed-forward scope: Cohort/runtime/registry/supplement=`51 passed + 11 subtests`; mandatory-PG CS5=`1 passed`; exact legacy recovery=`1 passed`; full supplement adjacency=`9 passed`; Ruff, Python compilation, and diff checks are green. The request-free product contract gate is `357 passed + 503 subtests`; global mypy remains exactly `81 errors / 4 files`.
- The valid pinned Ultra review of `7d54a23..60e7e67` was formal `NO-GO` with `P0/P1/P2/P3=0/6/4/0`. The first author fixed-forward was `17e1607` + `c6c0fcd`; its pinned local advisory at `04dd41c` returned `ADVISORY NO-GO 0/2/2/0`. `d7cb306` closed compatibility preflight, canonical identity, supplement fallback, and durable serving proof, but its pinned re-review returned `ADVISORY NO-GO 0/1/1/0` for persisted-state marker bypass plus committed-generation mutation and missing actual-boundary coverage. The Cohort path-filtered portion of `fd50a84` fixes those remaining families; focused evidence is `43 passed + 11 subtests`, exact legacy recovery `1 passed`, and mandatory-PG CS5 `1 passed`. This is author evidence pending fresh review. The reviewer-exclusive home proved `gpt-5.6-sol/ultra/priority`, but formal attempts remain unavailable after `usageLimitExceeded`; no unavailable review is represented as `GO`.
