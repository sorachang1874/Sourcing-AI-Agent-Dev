...
- [TRACK_D_D3C2B_SCOPED_ROOT_MIGRATION_IMPLEMENTATION.md](TRACK_D_D3C2B_SCOPED_ROOT_MIGRATION_IMPLEMENTATION.md) — D3c2b 非 live dormant scoped-root foundation（Migration A root subbatch：`plan_review_sessions` 11 columns + `operation_runs` 5 columns、合计 16 `NOT VALID` checks、5s both-table rollback；legacy mappers 不暴露新列，零 repository/writer/claim/served 激活；Activity/event/receipt/quarantine fragments 与 rollout step 3 继续 open）。
