...
- [TRACK_C_SERVING_RUNTIME_PLAN.md](TRACK_C_SERVING_RUNTIME_PLAN.md) — Track C serving runtime 排序计划（2026-07-14；C1+C2 已完成；C3a same-host API-never-runs-recovery 已落地并完成 pinned-review 后的 fixed-forward、re-review pending：serve external-daemon gate、provider/operator API signal-only；C3b 跨容器 singleton + durable outbox/5d 仍受 owner/residual gate；后续 C4 OpenAPI+SSE → C5 并入 Track D → C6 later）。
