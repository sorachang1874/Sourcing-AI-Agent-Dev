...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）已完成，storage.py 19,187 → 14,635 行。**下一批：②.3 serving_projection**（handbook §3 顺序；②.2 批数字见 Track B doc §6 末条）。Claude 无效尝试保留在 R-013；owner 2026-07-10 改为接受独立只读 Codex reviewer session，②.2 Codex review 发出后不阻塞 ②.3，结果回来再异步修复。
