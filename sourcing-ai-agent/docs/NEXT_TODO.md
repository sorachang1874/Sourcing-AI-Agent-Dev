...
- [ ] 最小鉴权 + 用户身份（token；`requester_id/tenant_id` 列已存在但来自未认证 payload）。
  - [x] C2.7 author fixed-forward（2026-07-14）：C2.6 scoped review 的 route-totality、daemon global-detail、read→write TOCTOU、Stage2 busy-lock 与 CRM blank-owner 文档 findings 已闭合。criteria 三路 rerun 传播 exact owner；authenticated daemon status 走独立 allowlist，只探测 canonical `job-recovery-<job_id>`，shared signal-only 细节既不探测也不返回；Stage2/cancel 走 PG owner-CAS；CRM PATCH 为 record+engagement+event owner-row-lock UoW，promotion 首写同锁；blank owner 继续遵守 C2.5 exact-workspace 兼容合同。验证为 fast `100+65 subtests`、PG `64+4 subtests`、transport `13`、CRM adjacency `2`、CRM runtime boundary `34`、lint `58 files`、mypy 棘轮 `81/4`。无 schema/provider/model/live；R-028 identity-lock/unique constraint/command-effect 边界仍 pending。fixed-forward formal review pending，有效 `GO` 前不做本 scope live/manual/product/里程碑签收。
