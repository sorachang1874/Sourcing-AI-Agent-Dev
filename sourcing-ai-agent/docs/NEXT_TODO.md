...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a catalog（`d97d17c`）+ ②.3b projection_manifest_shards 已完成，storage.py 19,187 → 14,261 行。②.2 与 ②.3a 独立 Codex review 均已 GO；②.3a 两条 low 测试耐久性建议下一 follow-up commit 清账。**下一批：②.3c serving_projection_members**；随后 ②.3d search-index，3d 后整域闭合。D-4 positional bulk-write fail-closed 范围待 owner 裁决，实施方向未定前只做 3c 只读 Scout/测试设计，不做会隐式选择方案的产品修改。各子批评审异步、scope-local，不阻塞其他模块开发。
