...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a-d serving_projection（catalog / manifest / members / person search index）+ ②.4a operation-control（`93d9f9e`）+ ②.4b acquisition-control（`d6e1e2a`）已完成，`storage.py` 19,187 → 12,564 行。②.3d 的三键 index/facet fence 待 R-017 pinned re-review；②.4a 已闭合 reject/cancel UoW 与 retry identity，R-018 待 pinned review、完整跨表 retry/dispatch 原子性留 R-019；②.4b 已闭合 6 facade + 2 mapper，单行 identity/JSON merge/terminal fence 已实测，跨 run/lane/activity/command cancel UoW 留 R-020，pinned 异步评审登记为 R-021。所有 review 均异步、scope-local，不阻断无关开发。**下一批：②.4c activity spine + R-020 fixed-forward**，按 handbook §4 重新 Scout。
