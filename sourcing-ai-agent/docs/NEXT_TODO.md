...
- [ ] 最小鉴权 + 用户身份（token；`requester_id/tenant_id` 列已存在但来自未认证 payload）。
  - [x] C2.7 author fixed-forward（2026-07-14）：C2.6 scoped review 的 route-totality、daemon global-detail、read→write TOCTOU、Stage2 busy-lock 与 CRM blank-owner 文档 findings 已闭合；后续 adversarial review 指出的四类问题也已 fixed-forward。criteria 三路在任何 feedback/pattern/version/compiler 写入前共享 read-only exact-owner preflight，显式/来源 baseline missing 或 foreign 为零写 `404`，automatic baseline 仍按 requester+tenant scoped。Stage2/cancel 改为 PG row-lock typed owner/state CAS（`applied|owner_miss|state_conflict`），精确约束 waiting/not-queued 与 nonterminal 并投影授权后的业务状态，不再让旧快照覆盖终态。CRM PATCH 的 stale version 与 Public-Web promotion immutable-idempotency conflict 均为 HTTP `409`；promotion 固定 record→schema-scoped promotion-id advisory→promotion row 锁序，cross-workspace/record/signal/action drift 不可改写，exact replay 保留原行。authenticated daemon allowlist 与 C2.5 blank-owner 合同不变。验证为 fast+transport `109+65 subtests`（transport `15`）、PG `71+4 subtests`（focused `9`）、CRM adjacency `2`、CRM runtime boundary `34`、pipeline exact `4 pass/2 clean-97a81d0-identical`、lint `58 files`、mypy 棘轮 `81/4`。无 schema/provider/model/live；R-028 legacy identity-lock/unique constraint/command-effect 以及 legacy promotion writer 统一锁边界仍 pending。fixed-forward formal review pending，有效 `GO` 前不做本 scope live/manual/product/里程碑签收。
