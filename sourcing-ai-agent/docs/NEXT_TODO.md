...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a catalog（`d97d17c`）+ ②.3b projection_manifest_shards（`aaf13fb`）+ ②.3c serving_projection_members（2026-07-10，`219f2e8`）已完成，storage.py 19,187 → 13,822 行。②.3c 将 8 public / 97 direct 全部迁移并清零旧 facade/dispatch；owner 选择的 D-4(a) 也已闭环：4 个 positional bulk-write 生产调用点全部改显式 keyword，并有全局 AST guard + direct/alias/getattr/unbound/computed 5-shape runtime bypass 变异自检。②.3b scope=`b3818f0..aaf13fb` 仍因 runner artifact 无效而由 R-014 scope-local 阻断签收，不阻塞开发。**下一批：②.3d person search index**，完成后 serving_projection 域闭合；各子批 review 继续异步、scope-local。
