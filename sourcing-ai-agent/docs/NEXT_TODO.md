...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a serving_projection catalog 三表已完成，storage.py 19,187 → 14,355 行。②.2 独立 Codex review 已 GO、R-013 closed。**下一批：②.3b projection_manifest_shards**；随后 ②.3c members / ②.3d search-index，3d 后整域闭合。各子批评审异步、scope-local，不阻塞下一子批开发。
