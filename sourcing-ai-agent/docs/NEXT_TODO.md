...
- [ ] 最小鉴权 + 用户身份（token；`requester_id/tenant_id` 列已存在但来自未认证 payload）。
  ...
  - [x] C2.8 author fixed-forward（2026-07-14）：authorization 已从 rerun policy 拆开；feedback、explicit recompile 两种 id spelling、suggestion source job 在任意主域写前无条件 exact-owner preflight。missing/foreign 同一 `job_not_found`；automatic baseline requester+tenant scoped、no-ref 与 open-mode 正向不变。验证为 fast/transport `74+61 subtests`、PG adjacency `9`、lint `58 files`、mypy 棘轮 `81/4`、py_compile/diff-check clean；无 pipeline full/provider/model/live。fresh pinned non-author review 待提交后发起；有效 `GO` 前只冻结本 scope live/manual/product/里程碑签收。
