...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a catalog（`d97d17c`）+ ②.3b projection_manifest_shards（`aaf13fb`）已完成，storage.py 19,187 → 14,261 行。②.2/②.3a Codex review 已 GO，②.3a low 已清账；②.3b scope=`b3818f0..aaf13fb` 的首个 reviewer 在 artifact 写入时撞 model capacity，两个 fallback 亦未产 artifact，R-014 pending（只阻断该 scope 签收）。**下一批：②.3c serving_projection_members**；调用面 Scout 已定 8 public / 97 direct（external production 56 + storage internal 2 + tests 39）及 hasattr 1 / patch 2 / fake 1。D-4 positional bulk-write fail-closed 范围待 owner 裁决，实施方向未定前不做会隐式选择方案的产品修改。各子批评审异步、scope-local，不阻塞其他模块开发。
