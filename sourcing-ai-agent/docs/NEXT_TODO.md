...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）+ ②.3a-d serving_projection（catalog / manifest / members / person search index）+ ②.4a workflow_runtime operation-control 已完成，`storage.py` 19,187 → 12,809 行。②.3d 的独立 review NO-GO 已 fixed-forward 为三键 index fence，R-017 待 pinned re-review；②.4a 已闭合 Store facade 与 reject/cancel UoW，并加 retry chain identity 修复，R-018 待 pinned review、完整跨表 retry/dispatch 原子性留 R-019。所有 review 均异步、scope-local，不阻断无关开发。**下一批：②.4b acquisition-control**，按 handbook §4 重新 Scout。
