...
- [ ] 剩余迁移盘点（2026-06-12 重新分类）：SIMPLE 大文件 ~9 个（含 `test_enrichment` 58 处实例化/38 个预算失败、`test_candidate_artifacts` 4.7k 行、`test_excel_intake`、`test_cloud_asset_import`、`test_company_asset_*`、`test_storage_profile_registry`、`test_runtime_rebuild`、`test_person_asset_crm_projection_contracts`、`test_target_candidate_public_web`、`test_asset_consolidation_audit`、`test_organization_execution_profile`）下一批；起 HTTP server/daemon 的 5 个（`test_cli`、`test_results_api`、`test_workflow_explain`、`test_worker_recovery_daemon`、`test_crm_public_web_runtime_boundary`）需 per-test context manager 变体；6 个已走 `PGDurableRuntimeTestMixin` 的可选统一；`test_pipeline`（42k 行，永不全量跑）最后单独设计；PG 适配器自测 3 个豁免。
