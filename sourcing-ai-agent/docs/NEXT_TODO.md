...
- [x] ② 域退役进行中（入口文档 `docs/TRACK_B_REPOSITORY_MIGRATION_HANDBOOK.md`）：②.0 linkedin_profile_registry（2026-07-02）+ ②.1 criteria/confidence（2026-07-06，`d5109f1`）+ ②.2 manual_review（2026-07-10，`d0828e7`）已完成，storage.py 19,187 → 14,635 行。**下一批：②.3 serving_projection**（handbook §3 顺序；②.2 批数字见 Track B doc §6 末条）。②.2 非 GPT 参考评审因 `ConnectionRefused` 无 verdict，按 owner 指示本 session 不重试，显式延期为 R-013；无效 artifact：`runtime/reviews/20260710T021502Z_async-reference-track-b-2-2-manual-review.md`。
