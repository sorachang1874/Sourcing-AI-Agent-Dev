...
- [ ] 剩余迁移：巨型 3 个——`test_enrichment`（58 处实例化，与其 38 个预算失败合批修）、`test_candidate_artifacts`（4.7k 行）、`test_results_api`（27.8k 行，与其 36 个预算失败合批修）；6 个已走 `PGDurableRuntimeTestMixin` 的可选统一；`test_pipeline`（42k 行，永不全量跑）最后单独设计；PG 适配器自测 3 个豁免。
