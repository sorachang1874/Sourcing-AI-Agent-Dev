...
1. **S1 — compensation contract module. DONE 2026-07-24.** `src/sourcing_agent/pipeline_compensation_contract.py` + `tests/test_pipeline_compensation_contract.py` (**72 tests / 142 subtests green**, offline pure functions). `sourcing.pipeline.compensation_intent.v1` strict fail-closed parser (exact-version lookup, key allowlists at every level, no auto-upgrade), the 18-row compensation **seat table** (every row an EXISTING recovery-tick phase; 9 mirror `recovery_drain_registry` bindings one-for-one and the suite cross-checks the mirror), the validator battery `V_STAGE_compensable` / `V_SEAT_existing_owner` / `V_KEY_owner_derived` / `V_DELTA_only_anchor` / `V_LINEAGE_scoped` / `V_SIGNAL_self_reported` / `V_ATTEMPT_bounded`, the §4 ladder as a pure function (`resolve_compensation_outcome`), and the terminal `compensation_exhausted_needs_human` escalation record (`build_compensation_escalation` / `normalize_compensation_escalation`). Tick oracle untouched. ADDITIVE.
...
|---|---|---|---|
| **S1 — DONE 2026-07-24** | Compensation contract module: `sourcing.pipeline.compensation_intent.v1` strict fail-closed schema (exact-version lookup, key allowlists at every level — and **no schema slot anywhere for a payload, request body, budget, or dispatch target**, so the intent carries a pointer and has no payment authority of its own), the 18-row compensation seat table, the 7-row validator battery, the §4 ladder (`resolve_compensation_outcome`), and the terminal escalation record. New `src/sourcing_agent/pipeline_compensation_contract.py` + `tests/test_pipeline_compensation_contract.py` (72 tests / 142 subtests, offline pure functions). The three ruling-critical structural pins: **OQ6** — `stage="promote"` is rejected at strict-parse AND by `V_STAGE` with the explicit no-seat reason; **OQ5/R-019** — `V_SEAT` admits only existing recovery-tick phases and `V_KEY` admits only keys in the target owner's own command-type namespace, rejecting every compensation-authored form (`compensation*`, the schema id, a key embedding `intent_id`), so a NEW dispatch identity is inexpressible and the R-019 call-site count stays 24; **OQ7** — `attempt.max` is ceilinged at 3 and a spent ladder MUST already carry `compensation_exhausted_needs_human`, so a silent re-loop is inexpressible. ADDITIVE ONLY — orchestrator/enrichment/acquisition/`recovery_phases` untouched; tick oracle green untouched (nothing is wired into the tick — that is S3). Registered: backend-ci.yml offline block + `tests/lane_manifest.py` GH_LANE_FULL + `regression_matrix` paired mapping (the contract module **and** `recovery_drain_registry.py` → the contract suite; the module also routes to the tick oracle) + `[tool.mypy].files` / `run_python_quality.sh` lint targets (sanctioned mypy re-baseline, 0 errors). | tick oracle untouched (green, 18/18) | request recorded (trigger 1: new contract schema) — verdict blocks nothing before S3 |
