...
| T-005 | `test_pipeline.py` 非 recovery 带分片 B SUPERSEDED 87 件（legacy HTTP-API 带 28701-29604 全带、feedback/confidence 带 28302-28632 全带、其余逐件）| legacy /results 形状、反馈/置信、及分片 B 表列明的各面 | 现代套件逐件确认同合同覆盖——**逐件证据行在 `NONBAND_OWNERSHIP_R2_SHARD_B_2026-07-22.md`**（法证方法同 T-004；特别注记:test_http_api_smoke 若移植会错校准到投影切断前的 legacy /results 形状）| 2026-07-22 | 见分片 B 表 evidence 列；43 件 SALVAGE 按 14 港组移植后才可续删 |
