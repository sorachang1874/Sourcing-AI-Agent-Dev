...
| `tests/test_completed_reconcile_open_work.py`（salvage waves 5+6, 2026-07-22）| completed-workflow 后台 reconcile（tick 发现 pending、exploration 结果 reconcile、completed-worker 快照胜 stale result view）+ open-work 分类边界（provider-owned refill 尾不算 daemon-owned ×2）+ 死 lease preflight 修复 | 从冻结 `test_pipeline.py` 移植 ×6；2 件入深法证队列（**疑真回归**：harvest reconcile 第二次调用未被 consumed 标记拦截而再次 reconcile——需查重复合并风险；board_visible daemon-owned 计数多 1 非 daemon 项）；棘轮 352→346 | PG fixture |
