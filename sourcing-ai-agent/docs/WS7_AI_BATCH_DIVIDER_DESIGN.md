...
**ENGAGEMENT EVIDENCE — the SEAM is real, the DATA is synthetic.** `scripts/ws7_shadow_divergence_report.py engagement` provisions its own ephemeral `sourcing_test_ws7_*` PG schema, builds a real `MultiSourceEnricher` with a scripted divider client, and calls `queue_background_profile_prefetch` with `execute_profile_refill_submit_commands=False` (zero provider calls). What this proves is that the **production call chain fires and a record lands** — gates A0→A7 all cleared and the record appeared on the returned `refill_plan_items["ai_batch_division_shadow"]`. The **inputs are fabricated by the script** (600 generated `ws7-engagement-*` urls), so the numbers below are PATH evidence, not corpus evidence, and the `division_id` is a fresh per-run UUID that identifies nothing (earlier revisions of this doc quoted one as if it were an evidence identifier — withdrawn):

```
shadow_status = proposed   engaged = True   eligible_member_count = 600 (SYNTHETIC)
batch_count = 4            division_id = <fresh per run — not an evidence identifier>
...
**SHADOW SAFETY GATE (added 2026-07-25, adversarial remediation).** The S3 hook engaged on a bare capability probe, which is `True` for the REAL provider clients too. Its exposure is NOT the authoritative write path — nothing the record contains can change dispatch and the ladder plan is byte-identical with the shadow on or off — but an ungated real client would have made a **billed provider call plus up to the 20 s divider timeout per wave mint >300 eligible urls, taken while the scheduler lock is held**: a cost/latency exposure on a scheduling hot spot. It is now gated identically to the promote seam: `record_profile_prefetch_division_shadow` requires capability (`model_client_supports_batch_division`) **AND** permission (`divider_shadow_model_calls_permitted`). A client whose class declares `ws7_shadow_billing_free = True` (the scripted divider) is always permitted; anything else needs `SOURCING_WS7_DIVIDER_SHADOW_ALLOW_REAL_MODEL`, **OFF by default** and deliberately NOT one of the three live-provider gate vars — so turning the system live does not turn the shadow's model call on. Unsetting it is the kill switch. Pinned by `tests/test_profile_batch_division_shadow.py::RealModelShadowSafetyGateTest` (a real-capable, non-billing-free client reaching the seam with no opt-in makes ZERO calls).

**DIVERGENCE CORPUS — READ THE INPUT DISCLOSURE FIRST (`--source both`, live schema read-only, session `default_transaction_read_only = on`, scripted client only).**

> **These are NOT observed ready sets.** The live registry has **zero** rows in `refill_queue_state='ready'` (census re-probed on every run: `''`=7,513, `retry_wait`=2, `planned_dispatch`=1; terminal `completed`=7,295 / `fetched`=218). What is real is each wave's **url population, shard tokens and attempt/failure history**; the **queue state is FABRICATED as `ready` for every member**, because replaying the real terminal state collapses the plan to an empty wave and nothing could be measured at all. So a "4,297-member set" is a job token's **cumulative all-time membership**, not items ever simultaneously awaiting refill. The emitted JSON carries this verbatim under `input_reconstruction` (with the live census), and the earlier framing "76 **real** ready sets" was wrong on the input side and is withdrawn.
>
> **Provenance matters more than the totals.** 75 reconstructed sets = **10 live-PG wave groups + 65 on-disk snapshot rosters** (58 production runtime + 7 from `runtime/test_env_live`). A **disk case is a candidate roster, not a refill queue**: every candidate carrying a linkedin_url is taken as pending with no eligibility/already-fetched filter (unlike the production mint path), and its shard face is **synthesized from one `source_dataset` token** (1–2 values) versus 157–2,389 real shard tokens on the PG cases — `distinct_shard_token_count` is **not comparable across provenances**. One test-fixture directory (`openai/snapshot-company-roster-prefetch-defers`, n=1,110, whose path literal is built by `tests/test_worker_completion_pipeline.py`) was counted as a real set in the first run and is now excluded by the loader.

| metric | result |
|---|---|
| reconstructed sets (raw / distinct by exact member set) | **75** / **55** — the same roster is reachable through several job tokens and snapshot ids |
| engaged (OQ5 >300) | 75 / 75 |
| battery-VALID non-ladder divisions | **41** |
| battery-REJECTED | **34 — every one `V1_provider_envelope`**; provenance **33 disk + 1 PG** |
| `dispatched_membership_identical` | 0 / 41 — **FORCED BY CONSTRUCTION, zero information** (see below) |
| mean best-match Jaccard vs the ladder | mean 0.355, range 0.167–0.819 — **FORCED BY CONSTRUCTION** (see below) |
| ladder dispatched batch count | **4 in all 75 cases** (pinned to `available_new_worker_count`), sizes 50/75 |
| deferred items, **like-for-like under the same R5 bound** | ladder **150,576** vs AI **129,190** — the AI closes **21,386 = 14.2%**, and **122,415** of the ladder figure sits on battery-REJECTED sets where the AI produces nothing and the F5 fallback runs the ladder plan verbatim |
| AI batch count / sizes | 4–8 batches of 78–299 (recomputed on the corrected 75-case corpus); the *plan* covers 100% of eligible members in every valid case (a plan, not a dispatch — see the row above); envelope utilisation 0.26–1.00 |
| V9 rounds `ceil(batch_count/inflight)` | inflight 4: ladder 1, AI 1–2 · inflight 8: ladder 1, AI 1 |

**Two headline metrics are forced before any data is read and must not be read as findings.** `dispatched_membership_identical` compares the ladder's **post-R5 dispatched** partition against the AI's **whole-eligible-set** division; the ladder deferred ≥1 item in every case, so its dispatched set is always a proper subset and 0/41 was the only possible value. It carries **no** information about whether the two divisions are different partitions, and the earlier parenthetical claiming otherwise is withdrawn. Likewise best-match Jaccard: both slicers are contiguous over the same ordering, so the value is essentially the chunk-size ratio (ladder 4×50 vs AI 8×287 → 50/287 = 0.174, which is the observed floor). It measures the size difference between two formulas, not regrouping quality.

**What the battery evidence actually is (narrowed).** Exactly **one** validator ever returned a non-pass: `V1_provider_envelope` (34×). The other nine contribute their pass path only, and three **cannot fail on this input at all**: `V5_worker_budget` (the recorder computes `simulated_dispatched = min(batch_count, available)` and then asks V5 whether that exceeds `available` — false by construction), `V6_wave_mint_only` (fails only on a foreign live division id; pre-S4 no writer exists, so the live set is empty — the recorder's own docstring says so), and `V8_retry_isolation` (the fabricated `ready` state overwrites the 2 genuine live `retry_wait` rows, so `retry_wait_indices` is empty in 75/75 cases). The **R6 durable-wave-inherited-window skip** is also unexercised, because the replay zeroes the recorded wave fields. The emitted summary carries all of this under `battery_coverage`. The earlier claim that this corpus "evidences the VALIDATOR BATTERY" is therefore **narrowed to V1 plus the pass path of the rest** — and "legal-but-non-ladder shapes" is withdrawn too: the scripted divider is a contiguous near-equal split and the ladder is contiguous slicing, so only the chunk-size parameter varied.

**Finding D8 — CONFIRMED (not discovered) here, and its prevalence restated deduped.** V1 caps a batch at 300 and V2/V10 cap the wave at 8 batches, so no legal division exists for `n > 8 × 300 = 2,400`. **This fact was already committed two days earlier** in `ScriptedProfileBatchDividerModelClient`'s own docstring (`model_provider.py`, commit `e04b3a6`, 2026-07-23: "outside that range no division can satisfy V1+V2/V9 simultaneously … the mint site bounds windows before engaging the divider"), together with the remediation D8 proposes. Attributing D8 itself to this corpus overstated it; the corpus **confirms** the structural claim and adds one genuinely new thing — the **prevalence**:

* raw: 34 / 75 cases = 45.3% — **inflated by duplicate counting**;
* **deduped by exact member set: 22 / 55 = 40.0%**;
* the >2,400 band's 34 cases collapse to **22 distinct member sets** and to **16 near-duplicate clusters at Jaccard ≥ 0.9** (one google roster appears 7×, one anthropic 3×, one xai 3×, one openai 3×);
* and it is **33/34 disk-provenance**, i.e. driven by candidate rosters with a synthesized shard face, not by live refill data.

(An independent audit computing the same quantity over its own 76-case snapshot got 22/58 = 37.9%; the small delta is the case list — the test fixture removed here, four previously truncated disk sets included — and membership-key normalization. Both refute the 45% headline the same way.) The design question stands unchanged: either the mint site must window the ready set below 2,400 before engaging the divider (S3's stated assumption, still unenforced), or OQ5 needs an explicit *upper* engagement bound. It must be settled before S5.

**HONESTY — read before quoting any number above.** The AI side is `ScriptedProfileBatchDividerModelClient`, i.e. `clamp(ceil(n/300), 4, 8)` contiguous equal chunks. It reads inventory size and the provider envelope and *nothing else* — not shard mix, not failure history, not queue state, attempts or priority. Every "divergence" number above is `f(n)` versus `g(n, worker_budget)`: two arithmetic formulas. **This corpus evidences the PATH, the V1 rejection behaviour, and data-quality facts about the real populations — never AI judgment quality, and never the whole validator battery.** It says nothing about prompts, reasoning, hallucination, cost, latency, or the F2/F3 real-provider failure modes; it is not a historical replay (rows are current post-fix state); and its *input* queue state is reconstructed, not observed. A real-model corpus is a separate, operator-gated step (S6).
