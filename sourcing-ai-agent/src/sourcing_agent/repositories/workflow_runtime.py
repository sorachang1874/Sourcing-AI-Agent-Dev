...
AGENT_TOOL_RESULT_SLOTS = TableDescriptor(
    ...
    columns=(
        ...
        Column("owner_target_revision_token"),
...
class WorkflowRuntimeRepository(Repository):
    ...
    def prepare_start_acquisition_tool_result(
        ...
        *,
        ...
        """Build one exact acquisition-start terminal without mutating the result slot."""
        ...
        for table_name in (
            ...
            self._require_postgres_for_durable_runtime(table_name)
        reader = getattr(self._adapter, "prepare_start_acquisition_tool_result", None)
        if not callable(reader):
            self._raise_read_failure(
                table_name="agent_tool_result_slots",
                method_name="prepare_start_acquisition_tool_result",
                reason="native start result owner reader is unavailable",
            )
            raise AssertionError("unreachable after native start result reader failure")
        try:
            terminal = reader(
                table_name="agent_tool_result_slots",
                occurrence=occurrence,
        ...
        self._raise_read_failure(
            table_name="agent_tool_result_slots",
            method_name="prepare_start_acquisition_tool_result",
            reason="native start result owner reader returned no terminal result",
        )
        raise AssertionError("unreachable after native start result reader failure")
        ...
        *,
        ...
        if result is not None:
            ...
            response = {
                ...
                "attempt": self._agent_tool_result_attempt_from_row(payload.get("attempt")),
                "journal": self._agent_tool_result_journal_from_row(payload.get("journal")),
                "released_workflow_command": WORKFLOW_COMMANDS.from_row(payload.get("released_workflow_command")),
            }
            if payload.get("recovery_wakeup"):
                response["recovery_wakeup"] = dict(payload.get("recovery_wakeup") or {})
            return response
        self._raise_write_failure(
            table_name="agent_tool_result_slots",
            method_name="accept_start_acquisition_tool_result_uow",
            reason="native writer returned no result bundle",
        )
        raise AssertionError("unreachable after native start result write failure")
