...
from .search_seed_registry import (
    ...
    cohort_publication_is_committed,
    ...
    def apply_search_seed_workers_to_snapshot(
        ...
        *,
        ...
        cohort_publication_digest = str(
            existing_summary.get(COHORT_PUBLICATION_DIGEST_FIELD) or ""
        ).strip()
        committed_cohort_digest = cohort_publication_commit_digest_from_candidate_documents(snapshot_dir)
        if cohort_publication_digest or committed_cohort_digest:
            ...
            # A committed Cohort generation is immutable outside the compiler
            # publication transaction. Legacy worker reconciliation must not
            # rewrite its entries/summary or project a marker-less replacement.
            return {"status": "skipped", "reason": "cohort_publication_immutable"}
