...
class ControlPlaneStore:
        ...
        *,
        ...
        if self._control_plane_postgres_should_prefer_read("workflow_recovery_intents"):
            ...
            if self._control_plane_postgres_should_skip_sqlite_fallback("workflow_recovery_intents"):
                return {}
        raise RuntimeError(
            "postgres-only invariant violated for workflow_recovery_intents in upsert_workflow_recovery_intent: should_prefer_read "
            "returned False; legacy SQLite tail retired (B4)"
        )
        ...
        *,
        ...
        if self._control_plane_postgres_should_prefer_read("workflow_recovery_intents"):
            ...
            if self._control_plane_postgres_should_skip_sqlite_fallback("workflow_recovery_intents"):
                return []
        raise RuntimeError(
            "postgres-only invariant violated for workflow_recovery_intents in claim_workflow_recovery_intents: should_prefer_read "
            "returned False; legacy SQLite tail retired (B4)"
        )
    ...
    def get_workflow_recovery_intent(self, job_id: str) -> dict[str, Any]:
        ...
        if self._control_plane_postgres_should_prefer_read("workflow_recovery_intents"):
            ...
            if self._control_plane_postgres_should_skip_sqlite_fallback("workflow_recovery_intents"):
                return {}
        raise RuntimeError(
            "postgres-only invariant violated for workflow_recovery_intents in get_workflow_recovery_intent: should_prefer_read "
            "returned False; legacy SQLite tail retired (B4)"
        )
    ...
    def _linkedin_profile_registry_row_payload(
        ...
        *,
        ...
        now = _utc_now_timestamp()
        # Track B B4.2: the persisted column payload is now built by the typed descriptor's to_columns —
        # write normalization (strip text / non-negative ints / status default "queued") is declared once
        # in repositories/linkedin_profile_registry.py and proven byte-equivalent to this former hand-coded
        # builder. created_at/updated_at default to now here.
        return _LINKEDIN_PROFILE_REGISTRY_DESCRIPTOR.to_columns(
            {
                "profile_url_key": profile_url_key,
    ...
    def get_linkedin_profile_registry_backfill_run(self, run_key: str) -> dict[str, Any] | None:
        ...
        if self._control_plane_postgres_should_prefer_read("linkedin_profile_registry_backfill_runs"):
            ...
            if self._control_plane_postgres_should_skip_sqlite_fallback("linkedin_profile_registry_backfill_runs"):
                return None
        raise RuntimeError(
            "postgres-only invariant violated for linkedin_profile_registry_backfill_runs in get_linkedin_profile_registry_backfill_run: should_prefer_read "
            "returned False; legacy SQLite tail retired (B4)"
        )
