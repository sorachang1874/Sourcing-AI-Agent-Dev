...
class WorkerCompletionPipelineTest(PGControlPlaneStoreTestMixin, unittest.TestCase):
    ...
    # -- shard-B port groups E+H+I (2026-07-22): post-profile/full-snapshot
    # materialization queue policy (wait-for-completion without last_error,
    # release idempotency, no summary scan without an item), inline-reconcile
    # writer-lock discipline (prefetch runs OUTSIDE the per-job writer lock,
    # never blocking peer remote completion), and the terminal-noop harvest
    # closure family (skip/consume/disable-inline-board-visible).
    ...
    def test_snapshot_full_materialization_queue_waits_for_workflow_completion_without_last_error(self) -> None:
        ...
        # CALIBRATED 2026-07-22: the snapshot-full-materialization queue rides
        # the durable command surface now — wait-for-completion is a
        # snapshot.compaction.run command in retry_wait, not a
        # waiting_prerequisite materialization item.
        commands = self.store.list_workflow_commands(
            workflow_run_id=legacy_job_workflow_run_id(job_id)
        )
        compaction_commands = [
        ...
        self.assertEqual(len(compaction_commands), 1)
        self.assertEqual(str(compaction_commands[0].get("status") or ""), "retry_wait")
        # The wait reason now lives on the command surface (retry_wait above);
        # the old item-metadata reason slot is retired with the item carrier.
    ...
    def test_snapshot_full_materialization_release_clears_old_wait_error(self) -> None:
        ...
        # CALIBRATED 2026-07-22 (durable command surface): release requeues
        # the compaction command (out of retry_wait) rather than re-queuing a
        # materialization item with a cleared last_error.
        commands = self.store.list_workflow_commands(
            workflow_run_id=legacy_job_workflow_run_id(job_id)
        )
        compaction_commands = [
        ...
        self.assertEqual(len(compaction_commands), 1)
        self.assertIn(str(compaction_commands[0].get("status") or ""), {"queued", "claimed", "running", "succeeded"})
    ...
    def test_snapshot_full_materialization_queue_does_not_scan_summary_without_item(self) -> None:
        ...
        # CALIBRATED 2026-07-22: reason vocabulary follows the command surface.
        self.assertEqual(snapshot_queue["reason"], "no_ready_snapshot_full_materialization_commands")
        self.assertNotIn("enqueue_scan", snapshot_queue)
        sync_mock.assert_not_called()
        retrieval_mock.assert_not_called()
        # CALIBRATED 2026-07-22 (durable command surface): no item and no
        # compaction command may exist for this job.
        commands = self.store.list_workflow_commands(
            workflow_run_id=legacy_job_workflow_run_id(job_id)
        )
        self.assertEqual(
            [c for c in commands if str(c.get("command_type") or "") == SNAPSHOT_COMPACTION_RUN_COMMAND_TYPE],
            [],
        )
    ...
    def test_existing_local_apply_closure_consumes_harvest_profile_terminal_noop(self) -> None:
        ...
        # CALIBRATED 2026-07-22 (durable command surface): consumption closes
        # the local-apply command (succeeded), not a materialization item.
        commands = self.store.list_workflow_commands(
            workflow_run_id=legacy_job_workflow_run_id(job_id)
        )
        local_apply_commands = [
            ...
            for command in commands
            if str(command.get("command_type") or "") == LINKEDIN_LOCAL_PROFILE_DELTA_APPLY_COMMAND_TYPE
            and str(command.get("status") or "") == "succeeded"
        ]
        self.assertEqual(len(local_apply_commands), 1)
        # The item-metadata terminal-noop reason slot retired with the item
        # carrier; the command keeps the generic enqueue reason and the
        # consumption is proven by the succeeded status + completed_count.
        self.assertEqual(
            str(dict(local_apply_commands[0].get("payload") or {}).get("reason") or ""),
            "provider_worker_completed_needs_local_apply_closure",
        )
        worker_after = self.store.get_agent_worker(worker_id=worker.worker_id)
        assert worker_after is not None
        ingest_marker = dict(dict(worker_after.get("output") or {}).get("inline_incremental_ingest") or {})
