...
GH_LANE_FULL: list[tuple[str, str]] = [
    ...
    ("tests/test_pipeline_compensation_contract.py", "WS7/W7.4 S1 compensation contract: compensation_intent.v1 strict schema (no payload/dispatch slot anywhere — the intent carries a pointer, never payment authority) + the V_STAGE/V_SEAT/V_KEY/V_DELTA/V_LINEAGE/V_SIGNAL/V_ATTEMPT battery pinning OQ6 (promote scoped out — no recovery-tick seat), OQ5/R-019 (owner-derived idempotency key only — the contract can never mint a new dispatch identity) and OQ7 (attempt ceiling 3, a spent ladder MUST already carry compensation_exhausted_needs_human) + the §4 ladder and terminal escalation record (offline, pure functions)"),
