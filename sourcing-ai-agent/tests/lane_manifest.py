...
GH_LANE_FULL: list[tuple[str, str]] = [
    ...
    ("tests/test_profile_batch_division_shadow.py", "WS7/W7.2 S3 shadow integration: mint-seam hook records division/fallback beside the plan, dispatch byte-identical shadow on/off, completion-path non-invocation, exception isolation (offline) + the 2026-07-25 ANTI-INERTNESS RATCHET: a real queue_background_profile_prefetch run against a PG store must still produce a shadow record with a divider-capable client (every other test in the file calls the hook directly and would stay green if the seam went inert)"),
    ...
    ("tests/test_organization_promote_contract.py", "WS7/W7.3 S1 promote-decision contract: ai_promote_decision.v1 strict schema (model authors only {decision,reason,reason_code}; guard verdict structurally excluded) + V_LINEAGE/V_COMP/V_GEN/V_PROV/V_LIFECYCLE validator battery + the §2.1 conjunction (promote ⟺ guard-pass AND AI-approve AND all-validators-pass) + F1-F6 keep-incumbent audit shapes (offline, pure functions)"),
    ("tests/test_organization_promote_model_surface.py", "WS7/W7.3 S2 promote-judge model-invocation surface: judge_organization_asset_promotion Protocol method (Deterministic {} = F1 keep-incumbent; scripted judge behind SOURCING_SCRIPTED_ORGANIZATION_PROMOTE_JUDGE; OpenAI/Qwen raw-output + judge-scoped 20s timeout), judge_and_validate_promotion OQ4 contested-only gating + once-per-decision call, and every F1-F5/honest-reject mapping to keep-incumbent via the S1 battery (offline)"),
    ("tests/test_organization_promote_shadow.py", "WS7/W7.3 S3 promote-judge SHADOW integration: record_organization_promote_shadow at the upsert_organization_asset_registry_with_guard seam records an ai_promote_decision.v1 + ladder-divergence digest on the returned record's sibling ai_promote_decision_shadow key while authority stays 100% the rule ladder + storage guard; pins contested-engaged / non-contested-skip (no model call) / shadow-exception isolation / the additive key absent from the persisted row, and THE key regression — the authoritative-row outcome is byte-identical with the shadow on vs off (PG-backed); plus the 2026-07-25 ANTI-INERTNESS RATCHET: the real build_company_candidate_artifacts entrypoint must thread model_client to the seam (it did not before that date — the parameter was dead in production and only tests calling the seam directly ever saw a shadow record) and every caller holding a model client must keep supplying it"),
    ("tests/test_pipeline_compensation_contract.py", "WS7/W7.4 S1 compensation contract: compensation_intent.v1 strict schema (no payload/dispatch slot anywhere — the intent carries a pointer, never payment authority) + the V_STAGE/V_SEAT/V_KEY/V_DELTA/V_LINEAGE/V_SIGNAL/V_ATTEMPT battery pinning OQ6 (promote scoped out — no recovery-tick seat), OQ5/R-019 (the key must match its owner's real key GRAMMAR — note the earlier 'can never mint a new dispatch identity' claim was WITHDRAWN as false: an offline contract constrains shape, not derivation, so store-backed derivation is a blocking S3 precondition, D-C9) and OQ7 (attempt ceiling 3; a SPENT ladder may never carry a null terminal status — the fail-open shape — while both closing terminals stay legal so an escalated row can be retired, D-C14) + the §4 ladder and terminal escalation record (offline, pure functions)"),
