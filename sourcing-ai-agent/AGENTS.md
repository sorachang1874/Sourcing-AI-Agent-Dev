...
4. Run the reviewer in read-only mode. For Codex, use `codex exec --strict-config --sandbox read-only` with a prompt file or non-interactive stdin; inherit the newest-model/highest-effort settings from `~/.codex/config.toml`, do not add model/effort/tier CLI pins, and store the output plus hash-bound effective rollout evidence under `runtime/reviews/`. A valid artifact requires `reviewer_exit_code=0`, complete actual model/effort/tier evidence, and no model reroute.
